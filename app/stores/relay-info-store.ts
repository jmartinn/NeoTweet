...
interface RelayStore {
  ...
  getRelayInfo: (relayUrl: string) => InfoState | undefined;
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        getAllRelayInfo: () => Object.values(get().relayInfoRecord),

        removeRelayInfo: (relayUrl) =>
          set((state) => {
            const { [relayUrl]: _, ...remainingRelays } = state.relayInfoRecord;
